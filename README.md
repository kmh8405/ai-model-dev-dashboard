...
### 2026-07-22 — 같은 유형의 일시적 HF 조회 실패 1건 (재발, 자동 복구)

- **문제**: `text` config API가 34회 재시도(30분) 내내 400 → 실패 1건.
- **원인**: 위 2026-07-13 사고와 완전히 동일한 에러 시그니처(같은 URL, 같은 400). 다만 이번엔 딱 1건만 실패했고, 이후 크론/웹훅에서 바로 정상 복구 — 이번 지연은 훨씬 짧게(다음 트리거 전에) 끝난 것으로 보임.
- **해결**: 코드 수정 없음 — 이미 있는 재시도 예산·진단 로깅·크론 안전망이 의도대로 동작해서 저절로 해결됨. 이후 2주간(08-04 기준) 추가 실패 없음.
